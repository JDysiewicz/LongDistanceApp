...
const MessageList = ({ messages, scroll, user, partner }: any) => {
          ...
          {messages.map((msg: any) => {
            if (msg.admin) {
              ...
              return (
                <Message
                  key={
                    msg.id ? msg.id : Math.random().toString() + new Date().toLocaleString("en-GB")
                  }
                  sender={
                    msg.sender_partner_code === user.partner_code ? user.nickname : partner.nickname
                  }
                  time={msg.time_sent}
                  user={user}
                  partner={partner}
